...
The odd syntax of an asterisk deferencing a hash refers to a symbol in the
current I<symbol table>, which is the place in the current namespace which
contains globally-accessible symbols such as package globals, functions, and
methods.  Assigning a reference to a symbol table entry installs or replaces
the appropriate entry.  To promote an anonymous function to a method, assign
