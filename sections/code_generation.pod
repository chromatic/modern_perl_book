...
The simplest code generation technique code is to build a string containing a
...
I<typeglob> is the Perl hacker jargon for "symbol table".> deferencing a hash
refers to a symbol in the current I<symbol table>, which is the place in the
current namespace which contains globally-accessible symbols such as package
globals, functions, and methods.  Assigning a reference to a symbol table entry
installs or replaces the appropriate entry.  To promote an anonymous function
to a method, assign that function reference to the appropriate entry in the
symbol table.
...
do just this.  C<Class::MOP> is the library which makes C<Moose> (L<moose>)
