...
Ideas: applying roles to object instances (which is basically
generating code), generating classes from a database schema or XML
Schema structure, ... not all of these with concrete examples yet.
But metaprogramming is very cool :-).  Perhaps worth noting that it is
this very flexibility that has allowed Moose itself to be written.
