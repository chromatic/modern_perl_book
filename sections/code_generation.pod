...
huge mess, you can also build amazing things. For example, metaprogramming
...
which will do nothing. This simple example is deceptive, because getting
C<eval> right takes some work. You must handle quoting issues to include
...
free, either. Yet even with its limitations, this technique is simple and
useful.
...
I<typeglob> is Perl jargon for "symbol table".> dereferencing a hash refers to
a symbol in the current I<symbol table>, which is the section of the current
...
syntax errors). When it finishes, parsing will continue as if there had been no
interruption.
...
there is no implicit C<BEGIN> block. After parsing finishes, Perl will run code
...
methods, there's no simple way to create classes at runtime in Perl. Moose
