...
Throughout the lifetime of a Perl variable, it may contain values of different
types--strings, integers, rational numbers, and more. Instead of attaching type
information to variables, Perl relies on the context provided by operators
(L<value_contexts>) to determine how to handle values. By design, Perl attempts
to do what you mean--you may hear this referred to as I<DWIM> for I<do what I
mean> or I<dwimmery>.>--though you must be specific about your intentions. If
you treat a value as a string, Perl will do its best to I<coerce> that value
into a string.
...
string C<'0'> all evaluate as false values. All other values--including strings
which may be I<numerically> equal to zero (such as C<'0.0'>, C<'0e'>, and C<'0
but true'>)--evaluate as true values.

When a scalar has I<both> string and numeric components (L<dualvars>), Perl
prefers to check the string component for boolean truth. C<'0 but true'>
evaluates to zero numerically, but it is not an empty string, so it evaluates
...
well as when using a value or an expression as a hash key. The undefined value
stringifies to an empty string but produces a "use of uninitialized value"
warning. Numbers I<stringify> to strings containing their values; the value
C<10> stringifies to the string C<10>. You can even C<split> a number into
individual digits with:
...
or expression as an array or list index. The undefined value I<numifies> to
zero and produces a "Use of uninitialized value" warning. Strings which do not
begin with numeric portions numify to zero and produce an "Argument isn't
numeric" warning. Strings which begin with characters allowed in numeric
literals numify to those values and produce no warnings, such that C<10 leptons
leaping> numifies to C<10> and C<6.022e23 moles marauding> numifies to
C<6.022e23>.

X<C<Scalar::Util>>
X<C<Scalar::Util>; C<looks_like_number>>
...
them produces no "Argument isn't numeric" warning. Beware that Perl's ideas of
infinity and not a number may not match your platform's ideas; these notions
aren't always portable across operating systems. Perl is consistent even if the
rest of the universe isn't.
...
Stringifying a numeric value does not I<replace> the numeric value. Instead, it
I<adds> a stringified value to the internal representation, which then contains
I<both> components. Similarly, numifying a string value populates the numeric
component while leaving the string component untouched.
...
you expect. Your author can recall doing so twice in two decades. Even so,
knowing that this caching occurs may someday help you diagnose an odd
situation.
