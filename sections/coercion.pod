...
Throughout its lifetime, a Perl variable may contain values of different
types--strings, integers, rational numbers, and more. Rather than attaching
type information to variables, Perl relies on the context provided by operators
(L<value_contexts>) to determine how to handle values. By design, Perl attempts
to do what you meanN<Called I<DWIM> for I<do what I mean> or I<dwimmery>.>,
though you must be specific about your intentions. If you treat a variable
which happens to contain a number as a string, Perl will do its best to
I<coerce> that number into a string.
...
evaluates to zero numerically, but it is not an empty string, and so it
...
warning. Numbers I<stringify> to strings containing their values, so the value
...
begin with numeric portions also numify to zero and produce an "Argument isn't
...
which uses the same parsing rules as the Perl grammar to extract a number from
a string.
...
Stringifying a numeric value does not I<replace> the numeric value.  Instead,
it I<adds> a stringified value to the internal representation, which then
...
you expectN<Your author can recall doing so twice in fifteen years of
programming Perl.>, but knowing that this caching occurs may someday help you
