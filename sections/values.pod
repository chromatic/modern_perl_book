...
=begin programlisting
    ...
    He looked up. "Time is never on our side, child.
...
a leading space before C<END_HEREDOC>. Yet if you indent the identifier, Perl 5
...
the 6.2 standard. If you need to care about the differences between Unicode
versions, you probably already know to see U<http://unicode.org/versions/>.
...
with a specific Unicode encoding, Perl will use something called an I<IO layer>
to map between octets and characters. The I<mode> operand of the C<open>
builtin allows you to specify an IO layer by name. For example, the C<:utf8>
...
If you have an existing filehandle to which you need to apply an IO layer,
modify it with C<binmode>:
...
bug stays hidden, because both encodings have the same representation for every
character.
...
perfect accuracy, Perl 5--like most programming languages--represents them
internally in a binary format. This representation is sometimes imprecise in
specific ways; consult C<perldoc perlnumber> for more details.
