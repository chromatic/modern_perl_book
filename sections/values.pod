...
introduce the heredoc.  The quotes are optional and determine whether the
heredoc obeys single-quoted or double-quoted behavior with regard to variable
and escape character interpolation.  The C<END_BLURB> itself is an arbitrary
...
=head4 Unicode and Strings

X<unicode>

Unicode is a system for representing characters in the world's written
languages.  While most English text uses only 127 characters (and thus requires
seven bits of storage, which fits nicely into eight-bit bytes), it's
naE<iuml>ve to believe that you won't someday need an umlaut, for example.

Perl 5 handles Unicode internally, and mostly you can ignore that it does so.
Because there are several representations for I<how> to store Unicode, you have
to know what kind of data you receive from the external world and what kind of
data you want to emit to the external world, but inside Perl, all of the string
operations you expect work just fine on Unicode data as opposed to ASCII
dataN<Provided you don't assume that a character in a string requires a byte of
storage.>.

For more details on what Unicode is, why encodings matter, and how to manage
incoming and outgoing data in a Unicode world, see C<perldoc perluniintro>.
...
is to use the C<utf8> pragma (L<pragmas>), which tells the Perl parser to read
the rest of the source code file in the UTF-8 I<encoding> (an arrangement and
representation of Unicode characters).  This allows you to use Unicode
characters in strings as well in identifiers:
...
When used on the left-hand side of an assignment, it enforces list context.  To
count the number of elements returned from an expression in list context
without using a temporary variable, you use the idiom (L<idioms>):
...
operator, Perl calls C<get_all_clown_hats()> in list context.  This produces a
list.  Even though that list never gets assigned to anything, that list is the
return value of that subexpression.  The assignment of that return value is in
scalar context because C<$count> is a scalar,
...
array and you may coerce an array to a list with the appropriate context, but
they are separate entities.  Lists may occur verbatim in source code as values:
...
Explain lvalue?  Link from here?

=end for

... as lvalue targets of assignments:
...
parentheses in these examples group expressions to change I<precedence> of
