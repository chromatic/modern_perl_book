...
next to each other, the first one will escape the second one.
...
become part of the string as if you'd performed a concatenation operation
directly:
...
C<open> builtin.  An I<IO layer> wraps around input or output and performs some
sort of conversion of the data.  In this case, the C<:utf8> layer performs
decoding of UTF-8 data:
