...
you give it meaning.  Perl strings can grow or shrink as you add to or remove
from them.
...
This demonstrates a useful principle: the syntax of a string may vary.  You can
represent a tab within a string with the C<\t> escape or by typing a tab
directly.  As Perl runs, both strings behave the same way, even though the
specific representation of the string may differ in the source code.
...
These escapes interpolate only within double-quoted strings.
