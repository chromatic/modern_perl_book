...
which uses an expensive calculation to produce many results.  If you call the
...
=begin programlisting
    ...
    my B<(>$single_elementB<)>   = some_expensive_operation();
...
through your whole task list.  In list context, the function can sort and
search and compare to produce an exhaustive list in the proper order.  If you
want all of that work, but only have time for a couple of tasks, you can use a
one or two-element list.
...
=begin programlisting
    ...
    (
        cheap_operation     => $cheap_operation_results,
        expensive_operation => B<scalar> some_expensive_operation(),
    );
