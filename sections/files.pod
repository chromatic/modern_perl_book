...
Most programs interact with the real world mostly by reading, writing, and
otherwise manipulating files. Perl began as a tool for system administrators
...
or output. Every Perl program starts with three standard filehandles, C<STDIN>
(the input to the program), C<STDOUT> (the output from the program), and
C<STDERR> (the error output from the program). By default, everything you
C<print> or C<say> goes to C<STDOUT>, while errors and warnings go to
...
Use the C<open> builtin to initialize a filehandle. To open a file for reading:
...
The first operand is a lexical which will contain the filehandle. The second
operand is the I<file mode>, which determines the type of file operation
(reading, writing, appending, et cetera). The final operand is the name of the
file on which to operate. If the C<open> fails, the C<die> clause will throw an
...
=item * C<< < >>, which opens a file for reading.

=item * C<< > >>, which open for writing, clobbering existing contents if the file exists and
creating a new file otherwise.

=item * C<<< >> >>>, which opens a file for writing, appending to any existing
contents and creating a new file otherwise.

=item * C<< +< >>, which opens a file for both reading and writing.

=item * C<< -| >>, which opens a pipe to an external process for reading.

=item * C<< |- >>, which opens a pipe to an external process for writing.
...
Assume the examples in this section have C<use autodie;> enabled so as to elide
explicit error handling. If you choose not to use C<autodie>, check the return
values of I<all> system calls to handle errors appropriately.
...
Perl to encode to or decode from a Unicode encoding. For example, to read a
file written in the UTF-8 encoding:
...
has to guess at what you mean, it may guess incorrectly. Worse, if C<$file>
came from untrusted user input, you have a potential security problem, as any
unexpected characters could change how your program behaves.
...
The special package global C<DATA> filehandle represents the current file of
source code. When Perl finishes compiling a file, it leaves C<DATA> open and
pointing to the end of the compilation unit I<if> the file has a C<__DATA__> or
C<__END__> section. Any text which occurs after that token is available for
reading from C<DATA>. The entire file is available if you use C<seek> to rewind
the filehandle. This is useful for short, self-contained programs. See
C<perldoc perldata> for more details.
...
or C<undef> if it's reached the end of file (test that condition with the
C<eof> builtin). Each iteration in this example returns the next line or
C<undef>. This C<while> idiom explicitly checks the I<definedness> of the
variable used for iteration, so only the end of file condition will end the
loop. This idiom is equivalent to:
...
C<for> imposes list context on its operands. When in list context, C<readline>
...
Perl assumes that files contain text by default. If you're reading I<binary>
data--a media file or a compressed file, for example--use C<binmode> before
performing any IO. This will force Perl to treat the file data as pure data,
without modifying it in any way, such as translating C<\n> into the
...
Given a filehandle open for output, C<print> or C<say> to write to the file:
...
filehandle contained in anything other than a plain scalar--a filehandle in an
array or hash or returned from an object method--and it won't hurt anything in
the simpler cases.
...
final argument to C<print> ( but always uses C<\n> as an implicit final
argument to C<say>). Remember that C<$\> is C<undef> by default. These two
...
a misnomer, however. C<$/> can contain any sequence of characters (but I<not> a
regular expression). This is useful for highly-structured data in which you
want to read a I<record> at a time.

Given a file with records separated by two blank lines, set C<$/> to C<\n\n> to
read a record at a time. Use C<chomp> on a record read from the file to remove
...
where Perl flushes C<STDOUT> every time it encounters a newline in the output.
...
method to change the buffering behavior of a lexical filehandle:
...
You can call any method provided by C<IO::File> on a filehandle. For example,
the C<input_line_number()> and C<input_record_separator()> methods do the job
of C<$.> and C<$/> on individual filehandles. See the documentation for
...
cannot I<write> to directories. Open a directory handle with the C<opendir>
builtin:
...
iterate over the contents of directories one entry at a time or you may assign
everything to an array in one swoop:
...
In a C<while> loop, C<readdir> sets C<$_>:
...
The names returned from C<readdir> are I<relative> to the directory itself.
(Remember that an I<absolute> path is a path fully qualified to its
filesystem.) If the F<tasks/> directory contains three files named F<eat>,
F<drink>, and F<be_monkey>, C<readdir> will return C<eat>, C<drink>, and
C<be_monkey> instead of F<tasks/eat>, F<tasks/drink>, and F<task/be_monkey>.

X<builtins; C<closedir>>

Close a directory handle with the C<closedir> builtin or by letting it go out
of scope.
...
The C<-e> operator has a single operand, either the name of a file or handle to
a file or directory. If the file or directory exists, the expression will
evaluate to a true value. C<perldoc -f -X> lists all other file tests.
...
C<-f> returns a true value if its operand is a plain file. C<-d> returns a true
value if its operand is a directory.  C<-r> returns a true value if the file
permissions of its operand permit reading by the current user. C<-s> returns a
true value if its operand is a non-empty file. Look up the documentation for
any of these operators with C<perldoc -f -r>, for example.
...
C<Path::Class> also provides convenience methods to remove files completely and
portably as well as to check certain file attributes.
...
The CPAN module C<File::chdir> makes manipulating the current working directory
easier.  If you're a fan of the command line and use C<pushd> and C<popd>, see
also C<File::pushd>.
