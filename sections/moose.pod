...
(L<immutability>) suggests making all instance data C<ro> and passing all
relevant data into the constructor.  In the C<Cat> example, C<age()> might
still be an accessor, but the constructor could take the I<year> of the cat's
birth and calculate the age itself based on the current year, rather than
relying on someone to update the age of all cats manually.  This approach helps
to consolidate all validation code and helps to ensure that all created objects
have valid data.
...
object of another class if they provide the same external interface.
...
For a length comparison of roles and other design techniques such as mixins,
