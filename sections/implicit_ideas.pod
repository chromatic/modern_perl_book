...
Perl's looping directives (L<looping_directives>) default to C<$_> as the
...
C<ARGV> has one special case. If you read from the null filehandle,
Perl will treat every element in C<@ARGV> as the I<name> of a file to
open for reading. (If C<@ARGV> is empty, Perl will read from standard
input.) This implicit C<@ARGV> behavior is useful for writing short
programs, such as this command-line filter which reverses its input:
...
Perl's C<< <$fh> >> operator is implemented with the C<readline> function. That is 'C<readline $fh>' does the same thing. However, until Perl 5.10, the readline function had no default, so a bare C<readline> did not do the same thing as empty C<< <> >>. But ever since Perl 5.10, a bare C<readline> behaves just like C<< <> >>, so we can now use C<readline> everywhere. For historic reasons, C<< <> >> is still more common, but consider using C<readline> as a more readable alternative. You probably prefer 'C<glob '*.html'> to C<< <*.html> >>, right? Same idea.
