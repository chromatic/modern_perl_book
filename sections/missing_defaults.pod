...
When Larry and the other Perl hackers designed Perl 5 back in 1993 and 1994,
they didn't know how the next decade and a half would change the world, let
alone programming.  Perl 5 added many great new features, but it also kept
compatibility with the previous seven years of Perl releases, from Perl 1
through Perl 4.  To keep all of the existing programs working, Perl's default
behavior supported Perl 4.

Sixteen years later, the best way to write clean, maintainable, powerful, and
succinct Perl 5 code is very different.  The default behaviors sometimes get in
the way; fortunately, better behaviors are available.

=head4 The C<strict> Pragma
...
Ensure that something explains "pragmas".

=end for

The C<strict> pragma allows you to forbid (or re-enable) various language
constructs which offer power but also the potential for accidental abuse.  The
C<Modern::Perl> module enables everything in C<strict> by default.
...
possibility of unchecked or underchecked user input manipulating internal-only
data for malicious purposes.

Requiring variable declarations helps to prevent typos in variable names, and
it encourages proper scoping of lexical variables.  It's much easier to see the
...
=head4 The C<warnings> Pragma

The C<warnings> pragma controls the reporting of various classes of warnings in
Perl 5, such as attempting to stringify the C<undef> value or using the wrong
type of operator on values.  It also warns about the use of deprecated
features.
...
... but only if your program has somewhere executed the line C<use IO::Handle>.
The solution is to add this line to your programs so that lexical
filehandles--the objects as they are--behave as objects should behave.
Alternately, use a pragmatic module such as C<Modern::Perl> which loads
C<IO::Handle> for you.
...
This pragma entered the Perl 5 core as of Perl 5.10.1.  See C<perldoc autodie> for more information.
