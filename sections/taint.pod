...
Perl gives you tools with which to write programs securely.  You don't have to
do this, and you can easily ignore these tools, but if you pay careful
attention to them, you can gain a measure of safety.  These tools are no
substitute for careful thought and planning--they I<reward> caution and
understanding--but they do help you avoid subtle mistakes.
...
program, but if you use it to affect the outside world, Perl will require you
to untaint the data.  Using tainted data insecurely causes Perl to throw a
fatal exception.
...
opportunity to taint environment data.

=head2 Sources of Taint

Taint can come from two places: file input and the operating environment of the
program.  The former is easy to understand, in that anything you read from a
file or collect from users in the case of web or network programming is
tainted.  The latter is more subtle.  This includes any command-line arguments,
environment variables, and data from system calls.  Even operations such as
reading from a directory handle (opened with C<opendir()>) produces tainted
data.
...
its provided argument is tainted.
...
Removing taint is easy to understand but less simple in practice.  You must
extract known-good portions of the data with a regular expression capture; the
captured data will not be tainted.  If your user input consists of a US
telephone number, for example, you can untaint it with:
...
program can be.  If you take the opposite approach and try to I<deny> specific
items or forms, you run the risk of overlooking something harmful.  In the case
of security, Perl prefers that you disallow something that's safe but
unexpected than that you allow something harmful that looks safe.  Even so,
nothing prevents you from writing a capture for the entire contents of a
variable--but in that case, why use taint?

=head2 Removing Taint from the Environment

X<taint; removing sources of>

Taint from the program's environment comes from a couple of places.  One is the
superglobal C<%ENV>, which represents environment variables for the system.
This data is tainted because forces outside of the program's control can
manipulate values there.  In particular, any environment variable which
modifies how the shell finds files and directories is an attack vector.  A
taint-sensitive program should thus delete several keys from C<%ENV> and set
