...
Perl I<arrays> are I<first-class> data structures--the language supports them
...
elements at will. Arrays grow or shrink as you manipulate them.
...
C<$cats[0]> is an unambiguous use of the C<@cats> array, because postfix
...
When you care more about the relative position of an element in the array, use
a negative array index. The last element of an array is available at the index
C<-1>. The second to last element of the array is available at index C<-2>, and
so on:
...
complainN<So would the cats, Petunia especially.>.

You may assign any expression which produces a list in list context to an
array:
...
to manipulate array elements without using indices.
...
removing are optional; you may omit either behavior. The C<perlfunc>
description of C<splice> demonstrates its equivalences with C<push>, C<pop>,
C<shift>, and C<unshift>. One effective use is removal of two elements from an
array:
...
argument passed to the function. C<@dogs> will be emptyN<... but Rosie thinks
she's a cat, so it's not all bad.>.
...
... because parentheses merely group expressions. They do not I<create> lists.
To avoid this flattening behavior, use array references (L<array_references>).
