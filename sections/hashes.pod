...
Similarly, the C<values> operator returns a list of values of the hash:
...
Does C<each> sound like too much trouble?  This is a well-known caveat, but it
doesn't come up as often as you might expect.  Be cautious, but use C<each>
when you need it.
...
Also like arrays, you may access a list of elements of a hash in one operation.
...
... except that the hash slice initialization does not replace the existing
keys of the hash.

Similarly you may retrieve multiple values from a hash with a slice:
...
Note that, as with array slices, the sigil of the hash changes to indicate a
list context.  You can still tell that C<%addresses> is a hash by the use of
the curly braces to indicate keyed access.
...
Of course, the choice between the two approaches depends on your merge
strategy.  What if the same key occurs in both hashes?  The hash slice approach
always overwrites existing key/value pairs in C<%addresses>.

=end sidebar

=head4 The Empty Hash
