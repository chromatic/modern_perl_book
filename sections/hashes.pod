...
returns false if it's empty (if it contains no keys or values) and true
otherwise.
...
parser to treat the previous bareword as if it were a quoted word.  The
C<strict> pragma will not warn about the bare word, and if you have a function
with the same name as a hash key, the fat comma will I<not> call the function:
...
hash to C<undef>.  It is cheaper, both in terms of code and memory, to use only
the keys of a hash for storage.
