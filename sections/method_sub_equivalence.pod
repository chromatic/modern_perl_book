=head2 Method-Function Equivalence
...
If C<$o> were I<not> an C<Order> object, but instead a subclass or allomorph
(L<roles>) of C<Order> which overrode C<calculate_price()>, the
method-as-a-function call would call the I<wrong> method.  As well, if the
internal implementation of C<calculate_price()> were to change--perhaps
inherited from elsewhere or delegated through C<AUTOLOAD()>--the caller might
break.
...
they perform particular manipulations to make sure that any object state the
function needs to access is available.  If the first argument is not a likely
