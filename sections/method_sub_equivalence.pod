...
Perl 5's object system is deliberately minimal.  As L<objects> explains, it
provides a very thin layer over existing Perl features.  This is nowhere more
obvious in the equivalence of Perl 5 subroutines and Perl 5 methods.

Because a class is a package, Perl itself makes no strong distinction between a
subroutine stored in a package and a method stored in a package.  The same
...
Perl itself will treat any subroutine of the appropriate name it can find in an
...
Note the use of C<sum()> from C<List::Util> to add the values of a list.
...
If C<$o> were I<not> an C<Order> object directly, but a subclass of C<Order> or
an allomorph of C<Order> (see L<roles>) which overrode C<calculate_price()>,
the method-as-a-function call would call the I<wrong> method.  As well, if the
internal implementation of C<calculate_price()> were to change, such that
C<Order> inherits it from an ancestor class or defines it lazily with
C<AUTOLOAD()>, the behavior might break in odd and difficult to debug ways.
...
method resolution without performing dispatch, how do you invoke the resulting
method reference?
...
When C<$meth_ref> contains a subroutine reference, Perl will invoke that
...
method.  Put another way, if the C<Order> class has changed such that
C<Order::apply_discount> is no longer the right method to call, the reference
in C<$meth_ref> will not have updated.
...
Because Perl 5 makes no distinction between subroutines and methods at the
point of declaration and because it's I<possible> (however inadvisable) to
invoke a given subroutine as a subroutine or a method, it's possible to write a
subroutine callable as either.

The core C<CGI> module is a prime offender with this technique.  Its functions
manually inspect C<@_> to determine whether the first argument is a likely
invocant.  If so, they perform particular manipulations to make sure that any
object state the subroutine needs to access is available.  If the first
argument is not a likely invocant, the subroutine must consult global data
elsewhere.
...
when considering that users can create their own subclasses.  As well, the
documentation burden is greater -- the explanation of a combined procedural and
object interface must reflect the dichotomy of the code -- and the potential
for misuse is greater.

What happens when one part of the project uses the procedural interface and
another uses the object interface?
