...
protection against calling functions that lack verification. Only those 
operations explicitly available in C<%dispatch> are available.
...
introspection available in the Perl debugger and in the C<caller> builtin show 
the difference:
...
wart on otherwise helpful syntax. # Indeed. Is there a reason why it's not there? (jeremiah)
