...
An anonymous function is a function without a name.  It behaves like a named
function--you can invoke it, pass arguments to it, return values from it, copy
references to it--do anything you can do to a named function with it.  The
difference is that it has no name.  You always deal with anonymous functions by
reference (see L<references> and L<function_references>).

=head4 Declaring Anonymous Functions

You may never declare an anonymous function on its own; you must construct it
and assign it to a variable or pass it as an argument to a function, either
explicitly or implicitly.  Explicit creation uses the C<sub> keyword with no
name:
...
You may use anonymous functions in place of subroutine references; as far as
Perl itself cares, they are equivalent.  Nothing I<necessitates> the use of
anonymous functions to perform these mathematical operations, but for functions
this short, there's little drawback to writing them this way.

Thus, you may rewrite C<%dispatch> as:
...
One benefit of the indirection of the dispatch table is that it provides some
protection against unrestricted user input calling functions elsewhere without
verification.  Only those operations explicitly available in C<%dispatch> are
available.
...
=head4 Anonymous Function Names
...
introspection such as is available in the Perl debugger and in the C<caller>
builtin show the difference:
...
inspect the names of functions, given function references.  C<sub_name()> is
the most immediately obvious in relation to C<caller>:
...
function allows you to give them names:
...
As the references section suggests (L<references>), changes to a referenced
function affect the referent.  That is, calling C<subname> on C<$anon> and
returning into C<$named> modifies the underlying function.  Both references
point to the same place.

=head4 Implicit Anonymous Functions

X<anonymous functions; implicit>

All of these anonymous function declarations have been explicit.  Perl 5 allows
implicit anonymous functions through the use of prototypes (see L<prototypes>).
...
argument.  This code is equivalent to:
...
wart on otherwise helpful syntax.
...
As discussed in L<prototypes>, this feature is occasionally useful despite its
drawbacks.  The syntactic clarity available by promoting bare blocks to
anonymous functions can be helpful, but use it sparingly and document the API
with care.
