...
Consider the builtin C<push>, which takes an array and a list. While Perl would
normally flatten the array and list into a single list passed to C<push>, Perl
knows to treat the array as a I<container> and does not flatten its values. In
effect, this is like passing a reference to an array and a list of values to
C<push>--because Perl's parser understands this is what C<push> needs to do.
...
include it for the full declaration--but the only reason to do so is to win a
trivia contest.
...
To see the prototype of a builtin, prepend C<CORE::> to its name for
C<prototype>'s operand:
...
Prototypes change how Perl parses your code and how Perl coerces arguments
passed to your functions. While these prototypes may superficially resemble
function signatures (L<function_signatures>) in other languages, they are very
different. They do not document the number or types of arguments functions
...
the limitations of the array prototype. That's a lot of cognitive burden to put
on a user--and if you think this error message is inscrutable, wait until you
see the I<complicated> prototype errors.
...
override a builtin. Finally declare your override with the correct prototype:
...
You may also usefully use prototypes to define compile-time constants. When
...
good effect to provide a nice API with delayed computation.
Its C<throws_ok()> function takes three arguments: a block of
...
C<Test::Fatal> allows similar testing and uses a simpler approach to avoid this
ambiguity.

X<builtins; C<sort>>

Ben Tilly suggests a final good use of prototypes, to define a custom named
function to use with C<sort>:
