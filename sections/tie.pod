...
hashes and arrays, so that Perl could use data too large to fit in available
memory. The core module C<Tie::File> allows you to do this, in effect treating
files as if they were arrays.
...
The first operand is the variable to tie. The second is the name of the class
...
C<Tie::StdScalar>, 
Then
override the specific methods for the operations you want to change. In the
case of a tied scalar, these are likely C<FETCH> and C<STORE>, possibly
C<TIESCALAR()>, and probably not C<DESTROY()>.
...
scalar. Assigning to that scalar reference changes the value of the scalar.
...
F<Tie/Scalar.pm>. That's right, there's no F<.pm> file for C<Tie::StdScalar>.
Isn't this fun?
...
behave as if they were builtin data types, avoid creating your own ties without
good reason. C<tie>d variables are also much slower than builtin data types.
