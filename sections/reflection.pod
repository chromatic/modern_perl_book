...
I<Reflection> is the process of asking a program about itself as it runs.
Whereas you can write many useful programs without ever having to use
reflection, techniques such as metaprogramming (L<metaprogramming>) benefit
...
useful programs do not use C<Class::MOP>.  Several idioms (L<idioms>) exist for performing reflection effectively in the absence of a formal system.

=head2 Checking that a Package Exists

To check that a package exists somewhere in the system--that is, if some code
somewhere has executed a C<package> directive with a given name--you can check
that the package inherits from C<UNIVERSAL>.  One easy way to do so is to check
that the package can perform the C<can()> method:

=begin programlisting

    say "$pkg exists" if $pkg->can( 'can' );

=end programlisting

Note that although you I<may> use packages with the names C<0> and C<''>N<...
only if you define them symbolically, as these are I<not> allowable identifiers
in source code.>, the C<can()> method will throw a method invocation exception
if you use them as invocants.  Wrap the invocation in an C<eval> block to
prevent this:

=for author

Need C<&& 1> here?  Only if someone's written a Liskov-violating C<can()>.
...
=begin programlisting
    ...
    $INC{'Modern/Perl.pm'} =>
        '/path/to/perl/lib/site_perl/5.12.0/Modern/Perl.pm';
...
(such as C<Test::MockObject> or C<Test::MockModule>) have their own reasons and
modules with poor reasons have other problems.
...
Note that C<$pkg> must be a valid invocant; wrap the method call in an C<eval>
block if you have any doubts about its validity.  Beware that a function
implemented in terms of C<AUTOLOAD()> (L<autoload>) may report the wrong answer
if the function's package does not also override C<can()> correctly.  This is a
bug in the other package.

You may use this technique to determine if a modules C<import()> has imported a function into the current namespace:
...
Rooting around in the symbol table to determine if a variable exists?  Icky.
