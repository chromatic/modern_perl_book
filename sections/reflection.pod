...
stores an entry in C<%INC> where the key is the file path of the module to load
and the value is the full path on disk to that module.  In other words, loading
C<Modern::Perl> effectively does:
...
The C<Class::Load> module from the CPAN provides a function named
C<is_class_loaded()> which can give better answers than checking C<%INC>
directly.
...
of the package.  For example, the symbol table for the C<MonkeyGrinder> package
is available as C<%MonkeyGrinder::>.
