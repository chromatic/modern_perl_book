...
zero (C<0>, C<0.0>, and C<0e0>) evaluate to false values. All other numbers are
evaluate to true values.

=begin tip What is Truth?

Be careful that the I<strings> C<'0.0'> and C<'0e0'> evaluate to true values.
This is one place where Perl 5 makes a distinction between what I<looks like> a
...
One other value is always a false value: C<undef>.
