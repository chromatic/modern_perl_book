...
value, an object, or a reference--but it is always a single value.  Scalar
values and scalar context have a deep connection; assigning to a scalar
provides scalar context.

Scalars may be lexical or global variables, and the names of scalar variables
must conform to the L<names> guidelines.  Scalar variables always use the
leading dollar-sign (C<$>) sigil (L<sigils>).
...
respecting character set and case.  Note that C<ZZ9> becomes C<ZZ0> and C<ZZ09>
becomes C<ZZ10>, however--numbers wrap around but do not carry to alphabetic
components.
...
place where Perl 5 makes distinction what looks like a number and what really
is a number.
