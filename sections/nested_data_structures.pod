...
This is one of Perl's uglier features. References are useful and you need to
understand them, but you don't have to like their syntax.
...
has one drawback you might find. Two references which point to each other
(directly or indirectly) form a I<circular reference> that Perl cannot destroy
on its own. Consider a biological model, where each entity has two parents and
...
When C<$alice> gets reclaimed, C<$cianne>'s reference to C<$alice> will be set
to C<undef>.
