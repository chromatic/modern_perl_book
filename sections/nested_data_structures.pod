...
Perl allows but does not require the trailing comma so to ease adding new
...
C<perldoc perldsc>, the data structures cookbook, gives copious examples of
various types of data structures available in Perl with their syntax.
...
but that can produce tedious, length code which many Perl programmers prefer to
avoid.
...
children of C<$alice> and C<$bob> or the parents of C<$cianne>), or take
