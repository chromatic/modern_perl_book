...
Finally, lexical scoping allows closures (see L<closures>).  Beware creating
...
C<2>, C<4>, and C<6>, the output is actually C<2>, C<3>, and C<4>.  This
behavior is as intended and documented, though its implementation can lead to
surprising results:
