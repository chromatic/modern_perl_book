...
Finally, lexical scoping facilitates closures (see L<closures>).  Beware creating
...
C<2>, C<4>, and C<6>, the output is actually C<2>, C<3>, and C<4>. The first call 
to the sub 'counter' sets the $count variable and it resists any attempts to 
change its value. This behavior is as intended and documented, though its 
