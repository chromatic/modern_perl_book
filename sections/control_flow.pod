...
Use the C<chomp> builtin to remove line-ending characters from each line.
Everyone forgets this at least once.
...
this problem in many ways; re-open the file inside the C<for> loop (simple to
...
Perl 5.10 introduced a new construct named C<given>. It was a nice idea, but it
didn't work out the way anyone hoped. The intent was to create a Perlish
C<switch> statement. C<given> is still experimental, but it's less buggy in
5.18 than it was in any previous version of Perl. That's a nice way of saying
"don't use it unless you know what you're doing."

X<builtins; C<when>>

If you need a switch statement, consider using C<for>'s aliasing behavior with
the C<when> statement. A C<when> statement matches the topic variable (C<$_>)
against expressions using smart match (L<smart_match>) semantics. To write the
Rock, Paper, ScissorsN<Adding Spock and Lizard is an exercise for the reader.>
game:
...
Unfortunately, Perl 5 does not automatically perform this optimization, so you
