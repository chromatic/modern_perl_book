...
The rarely used C<continue> construct may be used with a C<while>, C<until> or 
C<foreach> loop.  It behaves like the third subexpression of a C<for> loop -- 
it is evaluated for each iteration of the loop, even when a loop iteration is 
exited with a C<next>.  It can be used to increment a loop variable:
...
C<continue> can also be used with in a C<when> block, see below.
...
This seems like an rather verbose example.  Also, I'm not sure how to work 
mention of continue into it.  Perhaps something like this (pretty much ripped
from perldoc perlsyn) might suffice?

given($foo) {
    when (undef) {
        say '$foo is undefined';
    }
    when ([2, 3, 5, 7, 11, 13, 17, 19]) {
        say '$foo is a prime less than 20';
        continue; 
    }
    when ($_ % 2){
        say '$foo is odd';
    ...
    when ($_ < 20) {
        say '$foo is numerically less than 20';
    }
    default {
        say q{I don't know what to say about $foo};
    }
}
