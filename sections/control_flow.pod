...
You might also write a C<while> loop that does the I<destructive update> of 
the C<@values> array inside the C<while> condition check. This is a common
and idiomatic construct, but has some subtleties that aren't immediately
...
This loop will exit as soon as it reaches an element that evaluates to a
false or undefined value, not necessarily when it has exhausted the array.
That may be the desired behavior, but is often surprising to novices.
