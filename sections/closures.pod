...
... but be aware that you cannot I<nest> named functions.  Functions have
...
If all you ever need to do is to calculate Fibonnaci numbers, this approach may
...
Fibonnaci values, you can use this code to perform all sorts of cached, lazy
iterator behaviors.
...
=head3 Closures and Partial Application
...
Now instead of calling C<make_sundae()>, you can invoke C<$make_cart_sundae()>
and pass only the interesting arguments, without worrying about forgetting the
invariants or passing them incorrectlyN<You can even use C<Sub::Install> from
the CPAN to install this function into your namespace directly.>.
