...
Perl's builtin C<UNIVERSAL> package is the ancestor of all other packages--it's
the ultimate parent class in the object-oriented sense (L<moose>). C<UNIVERSAL>
...
appropriate class somehow does that role through inheritance, delegation,
composition, role application, or any other mechanism.
...
The C<can()> method takes a string containing the name of a method or function.
It returns a function reference, if it exists. Otherwise, it returns a false
value. You may call this on a class, an object, or the name of a package.
...
C<Module::Pluggable> makes it easier to build and manage plugin systems. Get to
know both distributions.
...
objects (C<Test::MockObject> and C<Test::MockModule>, for example) or with code
that does not use roles (L<roles>). Be aware that any class which I<does>
...
It's tempting to store other methods in C<UNIVERSAL> to make them available to
...
situations, there's no reason to put code in C<UNIVERSAL> directly, especially
given the other design alternatives.
