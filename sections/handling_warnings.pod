...
ways can be confusing, unclear, and even incorrect. Perl's warnings system can
...
similar, but it produces a backtrace of I<all> function calls which led to the
current function.
...
X<CPAN; C<Carp::Always>>

=begin tip Injecting Carp

Sometimes you'll have to debug code written without the use of C<carp()> or
C<cluck()>. In that case, use the C<Carp::Always> module to add backtraces to
all C<warn> or C<die> calls: C<perl -MCarp::Always some_program.pl>.
...
The venerable C<-w> command-line flag enables warnings throughout the program,
even in external modules written and maintained by other people. It's all or
nothing--though it can help you if you have the time and energy to eliminate
warnings and potential warnings throughout the entire codebase. This was the
only way to enable warnings in Perl programs for many years.

X<C<warnings>>
X<pragmas; C<warnings>>

The modern approach is to use the C<warnings> pragma (or an equivalent such as
C<use Modern::Perl;>). This enables warnings in I<lexical> scopes. If you've
used C<warnings> in a scope, you're indicating that the code should not
...
Code written before the C<warnings> pragma came about in spring 2000 may
C<local>ize C<$^W> to suppress certain warnings within a given scope.

=head2 Disabling Warning Categories

Use C<no warnings;> with an argument list to disable selective warnings within
a scope. Omitting the argument list disables all warnings within that scope.
...
Many warnings come from conditions that Perl can detect only when your code is
I<running>. If your test suite fails to identify all of the warnings you might
encounter, fatalizing these warnings may cause your program to crash. Newer
versions of Perl often add new warnings. Upgrading to a new version without
careful testing might cause new exceptional conditions.  More than that, any
custom warnings you or the libraries you use will also be fatal
(L<registering_warnings>).

If you enable fatal warnings, do so only in code that you control and never in
library code you expect other people to use.
...
exceptions. The C<%SIG> variable (see C<perldoc perlvar>) contains handlers for
out-of-band signals raised by Perl or your operating system. Assign a function
reference to C<$SIG{__WARN__}> to catch a warning:
...
C<%SIG> is a global variable, so  C<local>ize it in the smallest possible
scope.
...
The C<warnings::register> pragma allows you to create your own warnings which
users can enable and disable lexically. From a module, C<use> the pragma:
