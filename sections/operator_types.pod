...
operator returns a false value until its left operand is true, then it stays
true until its right operand is true, after which it is false again until the
left operand is true again.  Thus you could quote the body of a pedantically
