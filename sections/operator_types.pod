...
Perl's operators provide value contexts (L<value_contexts>) to their operands.
...
are the standard arithmetic operators of addition (C<+>), subtraction (C<->),
multiplication (C<*>), division (C</>), exponentiation (C<**>), and modulo
...
are C<&&>, C<and>, C<||>, and C<or>. These infix operators all exhibit
I<short-circuiting> behavior (L<short_circuiting>). The word forms have lower
precedence than the punctuation forms.
...
uncommon operators are left shift (C<< << >>), right shift (C<< >> >>), bitwise
and (C<&>), bitwise or (C<|>), and bitwise xor (C<^>), as well as their
in-place variants (C<< <<= >>, C<< >>= >>, C<&=>, C<|=>, and C<^=>).
...
component. If the value is obviously a string (if it has no numeric component),
...
evaluated in list context I<with a list as its first operand>, it evaluates to
that list repeated the number of times specified by its second operand. When
evaluated in list context I<with a scalar as its first operand>, it produces a
...
In boolean context, the range operator performs a I<flip-flop> operation. This
...
The fat comma operator (C<< => >>) also quotes any bareword used as its left
operand (L<hashes>).
