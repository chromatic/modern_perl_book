...
When given a scalar, it produces a string consisisting of the string value of
...
evalutes its left operand then returns the value produced by evaluating its
right operand.  In list context, it evalutes both operands in left-to-right
order.

The fat comma operator behaves the same way, except that it automatically
quotes any bareword used as its left operand.
