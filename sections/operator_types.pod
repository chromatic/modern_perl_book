...
Because of Perl's pervasive notion of context, especially value contexts
(L<value_contexts>), understanding an operator requires understanding the
context it provides to its operands.  The correct operator to use in a given
situation depends both on what you intend to do as well as on what type of
value you expect to use in that situation.
...
(C<^>), as well as their in-place variants (C<&=>, C<|=>, and C<^=>).
