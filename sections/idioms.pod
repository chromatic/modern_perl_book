...
and cringe at nasty hacks as we sling code.

Perl idioms are language features or design techniques. They're mannerisms and
mechanisms that give your code a Perlish accent. You don't have to use them,
...
binding lets you chain together multiple expressions to manipulate data in many
ways to produce the results you want.
...
Within the function, code unpacks these parameters into a hash and treats that
...
Perl to perform caller-side validation of the hash reference. In other words,
if you pass the wrong number of arguments, you'll get an error where you
I<call> the function.
...
list handling as an idiom borrowed from Lisp. Suppose you have a Perl hash
which associates the names of your co-workers with their phone extensions:
...
the leading zero, these keys look like octal numbers. Almost everyone makes
this mistake.
...
return 0. Finally, if C<$a> should come after C<$b> in the results, the block
...
The Schwartzian transformation uses Perl's pervasive list processing to get rid
...
when during Perl's compilation process--a variable you I<thought> would be
...
already begun. Yet a module and a program are both Perl code.  Making a module
executable is easy. So is making a program behave as a module (useful for
testing parts of an existing program without formally making a module). All you
need to do is to discover I<how> Perl began to execute a piece of code.
...
one expression in another expression. This isn't a new idea; you've likely used
the return value of a function in a list or as an argument to another function
before. You may not have realized its implications.
...
expression evaluates to the I<variable> C<$normalized_name>. That variable
becomes the first operand to the transliteration operator.
...
To ensure that Perl treats a value as a boolean, double negate it:
...
these idioms if you encounter them. While it may look like it would be safe to
remove a "useless" C<+ 0> from an expression, doing so may well break the code.
