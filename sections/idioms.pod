...
The second example is similar, except that it performs no assignment and merely
returns the single line read from the filehandle.  You may see either example;
they both work the same way in this case.
...
Calling C<main()> I<before> performing anything else in the program forces you
to be explicit about initialization and order of compilation.  It also helps to
...
=head3 Postfix Parameter Validation
