...
You may only declare one function of the same name per namespace, lest Perl 5
warn you about about subroutine redefinition.  If you're certain you want to
...
errors and throwing warnings in functions.  You may see C<croak()> used to
throw exceptions in place of C<die> in library code.  C<croak()> throws an
exception reported from the file and line number of its caller.  The C<cluck()>
function reports a warning from the file and line number of its caller.
...
I<Higher Order Perl>, by Mark Jason Dominus, is well worth your time.  See
...
be too few in certain circumstances but too many in others. You can disable
this warning with C<no warnings 'recursion'>.
