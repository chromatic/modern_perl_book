...
leading up to the point of the call.  It also captures the lexical environment
of the specific and current invocation of the function.  This means that a
function can I<recur>; it can call itself.
...
keyword.  Unlike the form which can often lead to spaghetti code, the C<goto>
...
has other behaviors which can occasionally surprise you.  First, it disables
prototype checking (as if that often mattered).  Second, if you do not pass
arguments explicitly, it I<implicitly> passes the contents of C<@_> unmodified.
Both can lead to surprising behavior.
