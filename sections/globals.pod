...
X<global variables; $INPUT_RECORD_SEPARATOR>
...
read the entire file into memory.  If you set this value to a I<reference> to
an integer, Perl will try to read that many I<bytes> per record (so beware of
Unicode concerns).

X<$.>
X<global variables; $.>
X<$INPUT_LINE_NUMBER>
X<global variables; $INPUT_LINE_NUMBER>

=item * C<$.> (C<$INPUT_LINE_NUMBER>) contains the current line (more
accurately, I<record>) number read from the most recently-accessed filehandle.
You can read from this variable, but writing to it has no effect.  Localizing
this variable will localize the filehandle to which it refers.

X<$|>
X<global variables; $|>
X<$OUTPUT_AUTOFLUSH>
X<global variables; $OUTPUT_AUTOFLUSH>
...
X<global variables; $">
X<$LIST_SEPARATOR>
X<global variables; $LIST_SEPARATOR>
