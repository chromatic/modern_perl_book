...
Perl provides several I<super global variables> that are truly global, not
scoped to a package or file. Unfortunately, their global availability means
that any direct or indirect modifications may have effects on other parts of
the program--and they're terse. Experienced Perl hackers have memorized some of
them. Few people have memorized all of them. Only a handful are ever useful.
C<perldoc perlvar> contains the exhaustive list of these variables.
...
you can avoid many of these globals. When you can't avoid them, use C<local> in
...
susceptible at least one race condition where C<DESTROY()> methods invoked on
...
if you don't explicitly read from those variables. Modern Perl programs can use
the C<@-> variable instead of them.

=head2 Useful Super Globals

X<super globals; useful>

Most modern Perl programs can get by with using only a couple of the super
...
C<$/> (or C<$INPUT_RECORD_SEPARATOR> from the C<English> pragma) is a
...
C<$!> (C<$ERRNO>) is a dualvar (L<dualvars>) which contains the result
...
overwriting the appropriate value for other code elsewhere. Perl's internals
make system calls sometimes, so the value of this variable can change out from
under you. Copy it I<immediately> after causing a system call to get accurate
results.
...
C<$"> (C<$LIST_SEPARATOR>) is a string used to separate array and list
...
C<$$> (C<$PID>) contains the process id of the currently running
instance of the program, as the operating system understands it. This will vary
between C<fork()>ed programs and I<may> vary between threads in the same
program.
