...
I<atom>). An atom either matches or it doesn't.
...
applies the regex of its second operand to a string provided as its first
operand. When evaluated in scalar context, a match evaluates to a boolean value
representing the success or failure of the match. The negated form of the
binding operator (C<!~>) evaluates to a true value I<unless> the match
succeeds.
...
corner store to buy cheese--but Perl lets you write code however you find it
clear.
...
and second delimiters) is a regular expression. The second operand (the part
between the second and third delimiters) is a substring used to replace the
matched portion of the string operand used with the regex binding operator. For
example, to cure pesky summer allergies:
...
The C<qr//> operator creates first-class regexes you can store in variables.
...
Matching literal expressions is good, but I<regex quantifiers> make regexes
more powerful. These metacharacters govern how often a regex component may
appear in a matching string. The simplest quantifier is the I<zero or one
quantifier>, or C<?>:
...
zero or one instance(s) of this atom." This regular expression matches if zero
or one C<a> characters immediately follow a C<c> character I<and> immediately
precede a C<t> character. This regex matches both the literal substrings C<cat>
and C<ct>.

X<regex; one or more quantifier>
X<C<+>; one or more regex quantifier>

The I<one or more quantifier>, or C<+>, matches at least one instance of its
atom:
...
than C<*>. Be precise about your intent to clarify your code.
...
input string as possible. This can be particularly pernicious. Consider a
...
quantifier as well as the range quantifiers. It causes the regex to match as
...
If you run this against all of the words in a dictionary, it'll match
C<Alabama>, C<Belgium>, and C<Bethlehem> long before it reaches C<loam>, the
real answer. Not only are those words too long, but the matched portions occur
everywhere in the word, not just at the start.
...
equally as important to know what kind of matches you want. I<Regex anchors>
force the regex engine to start or end a match at a fixed position. The I<start
of string anchor> (C<\A>) dictates that any match must start at the beginning
of the string:
...
circumstances it can match the invisible point just after a newline within the
string. Similarly, C<$> I<does> match the end of the string (just before a
newline, if it exists), but it can match the invisible point just before a
newline in the middle of the string. C<\A> and C<\z> are more specific and,
thus, more useful.
...
X<Unicode word boundary metacharacter>
X<C<\b{wb}>; word boundary regex metacharacter>

This anchor has one flaw which may or may not trip you; it doesn't understand
punctuation such as the apostrophe. Fortunately, Perl 5.22 added the I<Unicode
word boundary metacharacter> C<\b{wb}>, which I<does> understand contractions:
...
regex engine treats all metacharacters as atoms. See C<perldoc perlrebackslash>
...
Unicode alphanumeric characters (L<unicode>) and the underscore:
...
The C<\d> metacharacter matches Unicode digits:
...
boundary and C<\B{wb}> to match anywhere except a Unicode word boundary.
...
When none of those metacharacters is specific enough, group multiple characters
into a I<character class> by enclosing them in square brackets. A character
class allows you to treat a group of alternatives as a single atom.
...
To include the hyphen as a member of the class, place it at the start or end of
the class:
...
Note the escaped parentheses within C<$area_code>. Parentheses are special in
Perl regular expressions. They group atoms into larger units and capture
portions of matching strings. To match literal parentheses, escape them with
backslashes as seen in C<$area_code>.
...
regular expression and access them later. For example, when extracting a phone
number from contact information:
...
Perl also supports I<numbered captures>:
...
The I<first> matching capture goes into C<$1>, the second into C<$2>, and so
on. Capture counts start at the I<opening> parenthesis of the capture. Thus the
first left parenthesis begins the capture into C<$1>, the second into C<$2>,
and so on.

While the syntax for named captures is longer than for numbered captures, it
provides additional clarity. Counting left parentheses is tedious, and
...
in top-level regexes, rather than in smaller regexes composed into larger.
...
Remember to escape the alternation metacharacter (C<|>) as well as the end of
line metacharacter (C<$>) and the quantifiers (C<+>, C<?>, C<*>) if you want to
match their symbols literally.
...
when you don't control the source of match text:
...
Regex anchors such as C<\A>, C<\b>, C<\B>, and C<\Z> are I<regex assertions>.
These assertions do not match individual characters within the string. Instead
they match specific conditions of the string. For example, no matter what the
string contains, the regex C<qr/\A/> will I<always> match.
...
... but to find a non-disastrous feline, you could use a I<zero-width negative
...
The zero-width assertion consumes none of the source string, which leaves the
anchored fragment C<.*\Z> to match. Otherwise, the capture would only capture
...
To assert that your feline never occurs at the start of a line, use a
I<zero-width negative look-behind assertion>. These assertions must have fixed
sizes, and thus you may not use quantifiers:
...
Perl also includes the I<keep> assertion C<\K>. This zero-width positive
look-behind assertion I<can> have a variable length:
...
Everything until the C<\K> assertion matches, but only the portion of the match
I<after> the assertion will be substituted away.
...
ignore case distinctions. C<L> and C<l> are effectively equivalent in the
second regex due to the modifier.
...
Be aware that the C<\G> anchor will begin at the I<last> point in the string
...
The C</e> modifier allows you to write arbitrary code on the right side of a
substitution operation. If the match succeeds, the regex engine will use the
return value of that code as the substitution value. The earlier global
