...
Всеки език --човешки или програмен-- има общоприети изрази, наричани още идиоми. Земята
се върти, но ние говорим за изгреви и залези. Хвалим се и се умилкваме около мръсни и 
остроумни хакове докато размятаме код наляво и надясно.

Идиомите в Perl не са точно свойства на езика или пък техники на дизайн. Те са по-скоро
маниернечене и трикове, които, взети заедно, правят кода да изглежда повече като 
написан на Perl. Използването им не е задължително, но са част от силата на Perl. 

=head2 Обектът като C<$self>
...
Системата (L<moose>) за писане на обектно-ориентиран Perl 5 третира извикващия клас или
обект като обикновен параметър на метода. Независимо дали извиквате метод на клас или 
метод на обект, първият елемент от масива с параметрите C<@_> е това, което извиква 
метода - клас или обект. По конвенция в повечето Perl 5-код се използват имена на 
променливи като C<$class> и C<$self>, за да обозначат как се извиква даден метод. Тази 
конвенция е толкова силна, че разширения като C<MooseX::Method::Signatures> смело 
предполагат, че ще използвате C<$self> като име на променливата, съдържаща референция 
към обекта, извикващ метода. 

=head2 Именувани параметри
...
=begin tip Хеш или референция към хеш?
